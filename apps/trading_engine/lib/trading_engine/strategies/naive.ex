defmodule TradingEngine.Strategies.Naive do
  ...
  def init(config) do
    ...
    Logger.info(
      "Naive: Initialized for #{state.symbol} with trade_amount=#{state.trade_amount_usdt} USDT, buy_down=#{Decimal.mult(state.buy_down_interval, 100)}%, sell_up=#{Decimal.mult(state.sell_up_interval, 100)}%"
    )
    ...
    action =
      cond do
        ...
        state.position == nil and should_buy?(current_price, state) ->
          ...
          quantity =
            Decimal.div(state.trade_amount_usdt, current_price) |> Decimal.round(qty_prec)

          Logger.info(
            "Naive: Buy signal at #{current_price}, quantity=#{quantity} (#{state.trade_amount_usdt} USDT)"
          )
        ...
        state.position != nil and should_sell?(current_price, state) ->
          ...
          Logger.info(
            "Naive: Sell signal at #{current_price}, quantity=#{state.position.quantity}"
          )
